...
"""
Scraper de Eroski.
Utiliza la API interna de tienda.eroski.es / caprabo.com.
Requiere cookie de sesión configurada en .env (COOKIE_EROSKI).

TODO: Investigar la API de Eroski:
    1. Abrir https://supermercado.eroski.es/
...
def _get_headers():
    """
    Construye los headers necesarios para las peticiones a Eroski.
    ...
    return {
        ...
        'Cookie': os.getenv('COOKIE_EROSKI', ''),
...
def gestion_eroski():
    """
    Función principal que orquesta la extracción de productos de Eroski.
    
    Returns:
        pd.DataFrame: DataFrame con todos los productos de Eroski.
    """
    logger.warning("Scraper de Eroski pendiente de implementación.")
    logger.warning("Consulta los TODOs en scraper/eroski.py para instrucciones.")
    return pd.DataFrame()


def get_ids_categorys():
    """
    TODO: Obtiene la lista de IDs de todas las categorías de Eroski.
